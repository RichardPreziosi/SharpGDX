...
internal static class GLFW
{
	...
	public delegate void GLFWCursorPosCallback(long window, double xpos, double ypos);
	...
	public delegate void GLFWScrollCallback(long window, double xoffset, double yoffset);
	...
	public static void glfwGetFramebufferSize(long monitor, IntBuffer width, IntBuffer height)
	{
		var xHandle = GCHandle.Alloc(width.array(), GCHandleType.Pinned);
		var yHandle = GCHandle.Alloc(height.array(), GCHandleType.Pinned);
		glfwGetFramebufferSize(monitor, xHandle.AddrOfPinnedObject(), yHandle.AddrOfPinnedObject());

		xHandle.Free();
		yHandle.Free();

		[DllImport(Library)]
		static extern void glfwGetFramebufferSize(long window, long width, long height);
	}
	...
	{
		var xHandle = GCHandle.Alloc(widthMM, GCHandleType.Pinned);
		var yHandle = GCHandle.Alloc(heightMM, GCHandleType.Pinned);
		glfwGetMonitorPhysicalSize(monitor, xHandle.AddrOfPinnedObject(), yHandle.AddrOfPinnedObject());

		xHandle.Free();
		yHandle.Free();

		[DllImport(Library)]
		static extern void glfwGetMonitorPhysicalSize(long window, long widthMM, long heightMM);
	}

	public static void glfwGetMonitorPos(long monitor, IntBuffer xpos, IntBuffer ypos)
	{
		var xHandle = GCHandle.Alloc(xpos.array(), GCHandleType.Pinned);
		var yHandle = GCHandle.Alloc(ypos.array(), GCHandleType.Pinned);

		glfwGetMonitorPos(monitor, xHandle.AddrOfPinnedObject(), yHandle.AddrOfPinnedObject());

		xHandle.Free();
		yHandle.Free();

		[DllImport(Library)]
		static extern void glfwGetMonitorPos(long window, long xpos, long ypos);
	}
	...
	)
	{
		var xHandle = GCHandle.Alloc(xpos.array(), GCHandleType.Pinned);
		var yHandle = GCHandle.Alloc(ypos.array(), GCHandleType.Pinned);
		var widthHandle = GCHandle.Alloc(width.array(), GCHandleType.Pinned);
		var heightHandle = GCHandle.Alloc(height.array(), GCHandleType.Pinned);
		...
		(
			...
			widthHandle.AddrOfPinnedObject(),
		...
		[DllImport(Library)]
		static extern void glfwGetMonitorWorkarea(long window, long xpos, long ypos, long width, long height);
	}
	...
	public static Buffer glfwGetVideoModes(long monitor)
	{
		var count = IntBuffer.allocate(1);
		var countHandle = GCHandle.Alloc(count, GCHandleType.Pinned);

		var videoModes = glfwGetVideoModes(monitor, countHandle.AddrOfPinnedObject());
		...
		[DllImport(Library)]
		static extern long glfwGetVideoModes(long monitor, long count);
	}
	...
	{
		var xHandle = GCHandle.Alloc(xpos, GCHandleType.Pinned);
		var yHandle = GCHandle.Alloc(ypos, GCHandleType.Pinned);
		glfwGetWindowPos(window, xHandle.AddrOfPinnedObject(), yHandle.AddrOfPinnedObject());

		xHandle.Free();
		yHandle.Free();

		[DllImport(Library)]
		static extern void glfwGetWindowPos(long window, long xpos, long ypos);
	}

	public static void glfwGetWindowSize(long monitor, IntBuffer width, IntBuffer height)
	{
		var xHandle = GCHandle.Alloc(width.array(), GCHandleType.Pinned);
		var yHandle = GCHandle.Alloc(height.array(), GCHandleType.Pinned);
		glfwGetWindowSize(monitor, xHandle.AddrOfPinnedObject(), yHandle.AddrOfPinnedObject());

		xHandle.Free();
		yHandle.Free();

		[DllImport(Library)]
		static extern void glfwGetWindowSize(long window, long width, long height);
	}
	...
	[DllImport(Library)]
	public static extern bool glfwWindowShouldClose(long window);
