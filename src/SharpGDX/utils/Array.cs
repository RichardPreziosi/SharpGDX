 ...
 * @author Nathan Sweet */
	...
	public void addAll(Array< T> array, int start, int count)
	{
		if (start + count > array.size)
			throw new ArgumentException("start + count must be <= size: " + start + " + " + count + " <= " + array.size);
	...
	public bool contains([Null] T value, bool identity)
	{
		T[] items = this.items;
		int i = size - 1;
	...
	public int indexOf([Null] T value, bool identity)
	{
		T[] items = this.items;
		if (identity || value == null)
		{
	...
	public int lastIndexOf([Null] T value, bool identity)
	{
		T[] items = this.items;
		if (identity || value == null)
		{
	...
	public bool removeValue([Null] T value, bool identity)
	{
		T[] items = this.items;
		if (identity || value == null)
		{
	...
	{
		int n = size;
		if (end >= n) throw new IndexOutOfRangeException("end can't be >= size: " + end + " >= " + size);
	...
	public IEnumerator<T> GetEnumerator()
	{
		...
		return (ArrayIterator<T>)iterable.GetEnumerator();
	}
	...
	public T[] toArray()
	{
		return (T[])toArray<T>(items.GetType().GetElementType());
	}
	...
	public void Reset()
	{
		// TODO: Should this just throw a NotSupportedException?
	...
	public T? Current
	{
		get
		{
			if (index >= array.size) throw new InvalidOperationException(index.ToString());
			if (!valid)
			{
				// System.out.println(iterable.lastAcquire);
				throw new GdxRuntimeException("#iterator() cannot be used nested.");
