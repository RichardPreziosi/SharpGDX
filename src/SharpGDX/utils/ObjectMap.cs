...
// TODO: I don't know that '(V)dummy' will work.
 ...
 * @author Tommy Ettinger */
	public class ObjectMap<K, V> : IEnumerable<ObjectMap<K, V>.Entry<K, V>>
	{
		...
		[NonSerialized] Entries<K, V> entries1, entries2;
		[NonSerialized] Values<V> values1, values2;
		[NonSerialized] Keys<K> keys1, keys2;
		...
		public override bool Equals(Object? obj)
		{
			...
			for (int i = 0, n = keyTable.Length; i < n; i++)
			{
				...
				if (key != null)
				{
					...
					if (value == null)
					{
						if (other.get(key, (V)dummy) != null) return false;
					}
					else
					{
		...
		public IEnumerator<ObjectMap<K, V>.Entry<K, V>> GetEnumerator()
		{
			return entries();
		}
		...
		{
			if (Collections.allocateIterators) return new Entries<K, V>(this);
			if (entries1 == null)
			{
		...
		public abstract class MapIterator<K, V, I> : IEnumerator<I>, IEnumerable<I>
		{
			...
			protected readonly ObjectMap<K, V> map;
			...
			{
				int i = currentIndex;
				if (i < 0) throw new InvalidOperationException("next must be called before remove.");
				K?[] keyTable = map.keyTable;
				V[] valueTable = map.valueTable;
		...
		public class Entries<K, V> : MapIterator<K, V, Entry<K, V>>
		{
			...
			public override Entry<K, V> Current
			{
				get
				{
					if (!hasNext) throw new InvalidOperationException("You cannot move next");
		...
		public class Values<V> : MapIterator<K, V, V>
		{
			...
			public override Values<V> GetEnumerator()
			{
				return this;
			}
		...
		public class Keys<K> : MapIterator<K, V, K>
		{
			...
			public override Keys<K> GetEnumerator()
			{
				return this;
			}
