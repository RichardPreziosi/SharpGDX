 ...
 * in place, using a binary insertion sort. */
	...
	internal TimSort()
		...
		private static int gallopLeft(T key, T[] a, int @base, int len, int hint, IComparer< T> c)
		{
		...
		private static int gallopRight(T key, T[] a, int @base, int len, int hint, IComparer< T> c)
		{
			if (DEBUG) Debug.Assert(len > 0 && hint >= 0 && hint < len);
		 ...
		 * @throws ArgumentException if fromIndex > toIndex
