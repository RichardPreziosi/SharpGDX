 ...
 * @author Tommy Ettinger */
	public class ObjectSet<T> : IEnumerable<T>
	{
		...
		[NonSerialized] private ObjectSetIterator<T> iterator1, iterator2;
		...
		public void addAll(Array<T> array, int offset, int length)
		{
			if (offset + length > array.size)
				throw new ArgumentException("offset + length must be <= size: " + offset + " + " + length + " <= " +
				                            array.size);
		...
		public T first()
		{
			...
			for (int i = 0, n = keyTable.Length; i < n; i++)
				if (keyTable[i] != null)
					return keyTable[i];
			throw new InvalidOperationException("ObjectSet is empty.");
		}
		...
		{
			if (Collections.allocateIterators) return new ObjectSetIterator<T>(this);
			if (iterator1 == null)
			{
		...
		public class ObjectSetIterator<K> : IEnumerable<K>, IEnumerator<K>
		{
			public bool hasNext;
