...
public Gdx2DPixmap(Stream @in, int requestedFormat) // TODO: throws IOException
		{
		...
		public Gdx2DPixmap(byte[] pixelPtr, long[] nativeData)
		{
 ...
 */
		...
		private static extern byte[] load(long[] nativeData, byte[] buffer, int offset, int len); /*MANUAL
		...
		private static extern byte[] loadByteBuffer(long[] nativeData, byte[] buffer, int offset, int len); /*MANUAL
				if(buffer==0)
					return 0;
		...
		private static extern byte[] newPixmap(long[] nativeData, int width, int height, int format); /*MANUAL
