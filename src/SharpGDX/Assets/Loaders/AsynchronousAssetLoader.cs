...
internal interface AsynchronousAssetLoader : AssetLoader
{
 ...
 * @param parameter the parameters to use for loading the asset */
	void loadAsync(AssetManager manager, String fileName, FileHandle file, AssetLoaderParameters parameter);
 ...
 * @param file the resolved file to load
 * @param parameter */
	object loadSync(AssetManager manager, String fileName, FileHandle file, AssetLoaderParameters parameter);
 ...
 * method is called and must release any resources it allocated. */
	void unloadAsync(AssetManager manager, String fileName, FileHandle file, AssetLoaderParameters parameter);
}
...
* @param <T>
* @param <P> */
public abstract class AsynchronousAssetLoader<T, P> : AssetLoader<T, P>, AsynchronousAssetLoader
where P : AssetLoaderParameters<T>
{
	...
	object AsynchronousAssetLoader.loadSync(AssetManager manager, String fileName, FileHandle file, AssetLoaderParameters parameter)
	{
		return loadSync(manager, fileName, file, (P)parameter);
	}

	void AsynchronousAssetLoader.unloadAsync(AssetManager manager, string fileName, FileHandle file, AssetLoaderParameters parameter)
	{
		unloadAsync(manager, fileName, file, (P)parameter);
	}

	void AsynchronousAssetLoader.loadAsync(AssetManager manager, string fileName, FileHandle file, AssetLoaderParameters parameter)
	{
